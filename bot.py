...
def class_keyboard():
    ...
    return {
        ...
        "one_time_keyboard": True,  # Сворачивается сразу после первого нажатия
    }
...
# ============================================================
# YANDEX PDF DOWNLOAD (HYBRID MULTI-STAGE DOWNLOADER)
# ============================================================

def try_yandex_api(target):
    """Этап 1: Попытка прямого скачивания через Яндекс Диск Cloud API."""
    print("Этап 1: Проверка Yandex Cloud API...")
    key = SOURCE_URL.split("/")[-1]
    api_url = "https://cloud-api.yandex.net/v1/disk/public/resources/download"
    
    test_keys = [
        f"https://disk.yandex.ru/i/{key}",
        f"https://disk.yandex.ru/d/{key}",
        f"https://docs.yandex.ru/view/d/{key}",
        key,
    ]

    for k in test_keys:
        try:
            res = session.get(api_url, params={"public_key": k}, timeout=5)
            if res.status_code == 200:
                download_href = res.json().get("href")
                if download_href:
                    file_res = session.get(download_href, timeout=15)
                    if file_res.status_code == 200 and file_res.content.startswith(b"%PDF") and len(file_res.content) > 1000:
                        target.write_bytes(file_res.content)
                        print("Успех: PDF мгновенно скачан через Yandex Cloud API!")
                        return True
        except Exception:
            continue
...
def download_pdf():
    ...
    # Сначала пытаемся скачать напрямую через API без браузера
    if try_yandex_api(target):
        return target

    # Этап 2: Браузержная автоматизация через Playwright
    print("Этап 2: Скачивание PDF через браузер Playwright...")
    t0 = time.time()
    ...
    with sync_playwright() as p:
        ...
        # Обход встроенного детектирования ботов
        ...
        def handle_response(response):
            try:
                ...
                if "application/pdf" in ctype or url.endswith(".pdf") or ("filename=" in disp and ".pdf" in disp):
                    if response.status == 200:
                        body = response.body()
                        if body and body.startswith(b"%PDF") and len(body) > 1000:
                            pdf_network_data.append(body)
                            print(f"Перехвачен PDF из сетевого потока ({len(body)} байт)!")
            except Exception:
                pass
        ...
        try:
            ...
            print(f"Предупреждение при открытии страницы: {e}")

        page.wait_for_timeout(3500)

        # JS-скрипт прямого поиска и клика внутри DOM всех фреймов
        js_find_and_click = """
        (patternStr) => {
            const pattern = new RegExp(patternStr, 'i');
            function searchDoc(doc) {
                const nodes = Array.from(doc.querySelectorAll('button, a, div, span, [role="button"], [role="menuitem"]'));
                for (const el of nodes) {
                    const txt = (el.innerText || el.textContent || '').trim();
                    if (pattern.test(txt) && el.offsetWidth > 0 && el.offsetHeight > 0) {
                        el.click();
                        return true;
                    }
                }
                ...
                for (const iframe of iframes) {
                    try {
                        if (iframe.contentDocument) {
                            if (searchDoc(iframe.contentDocument)) return true;
                        }
                    } catch(e) {}
                }
                return false;
        ...
        for attempt in range(12):
            if pdf_network_data:
                target.write_bytes(pdf_network_data[-1])
                download_ok = True
                print("PDF успешно сохранен из сетевого потока.")
                break

            print(f"Попытка {attempt + 1}/12: Поиск элементов редактора...")

            # 1. Поиск и клик по "Файл"
            clicked_file = page.evaluate(js_find_and_click, "^Файл$")
            if not clicked_file:
                clicked_file = page.evaluate(js_find_and_click, "Файл")
            ...
            if clicked_file:
                ...
                # 2. Поиск и клик по "Скачать"
                clicked_download = page.evaluate(js_find_and_click, "^Скачать$")
                ...
                if clicked_download:
                    ...
                    # 3. Клик по "Документ PDF" с перехватом скачивания
                    try:
                        with page.expect_download(timeout=20000) as download_info:
                            clicked_pdf = page.evaluate(js_find_and_click, "Документ PDF|\\.pdf|PDF")
                            if not clicked_pdf:
                                raise RuntimeError("Кнопка PDF не найдена")

                        download = download_info.value
                        if not download.failure():
                            download.save_as(target)
                            download_ok = True
                            print("PDF скачан успешно через JS-автоматизацию!")
                            break
                    except Exception as e:
                        print(f"Задержка ожидания PDF: {e}")

            # Прямая кнопка "Скачать" на панели
            clicked_direct = page.evaluate(js_find_and_click, "^Скачать$")
            if clicked_direct and not download_ok:
                try:
        ...
        if not download_ok and pdf_network_data:
            target.write_bytes(pdf_network_data[-1])
            download_ok = True
